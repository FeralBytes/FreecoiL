...
var module_paths = [
    "res://tests/multiplayer_tests/NetworkingCode_gd.gd",
]
var full_logs = ""
...
func start_tests_as_server():
    ...
    print()
    log_to_full_log("[STARTING ALL MULTIPLAYER INTEGRATION TESTS]")
    ...
    for path in module_paths:
        ...
        the_tester = load("res://tests/TheMultiplayerIntegrationTester.gd").new()
...
func end_and_print_results():
    if all_test_failures == 0:
        ...
        yield(get_tree(), "idle_frame")
        log_to_full_log("[ALL MULTIPLAYER INTEGRATION MODULES & TESTS PASSED - TOTAL TEST SUCCESSES] " + str(all_test_successes) + "/" + str(count_all_tests))
        OS.set_exit_code(0)  # OK Exit
    else:
        rpc("clients_shutdown", 70)
        yield(get_tree(), "idle_frame")
        log_to_full_log("[SOME MULTIPLAYER INTEGRATION MODULES & TESTS FAILED - TOTAL TEST FAILURES] " + str(all_test_failures) + "/" + str(count_all_tests))
