...
def run_lan_time_test():
    ...
    print("#############################################################################")
    return returncode1.result(), returncode2.result(), returncode3.result()

def run_lan_death_test():
    executor = concurrent.futures.ThreadPoolExecutor(max_workers=3)
...
if len(sys.argv) == 2:
    if sys.argv[1] == "lan_time":
        results = run_lan_time_test()
        if results[0] == 0 and results[1] == 0 and results[2] == 0:
            ...
            sys.exit(1)
    elif sys.argv[1] == "lan_death":
        results = run_lan_death_test()
        if results[0] == 0 and results[1] == 0 and results[2] == 0:
            print("Success!")
            sys.exit(0)
        else:
            ...
            sys.exit(1)
    elif sys.argv[1] == "lan_ffa_death":
        results = run_lan_ffa_death_test()
        if results[0] == 0 and results[1] == 0 and results[2] == 0:
            print("Success!")
            sys.exit(0)
        else:
