...
func _ready():
    TimeTick.text = "In: " + "%03d" % SettingsConf.S.QuickStart.respawn_delay
...
func _on_TickTockTimer_timeout():
    current_ticks += 1
    TimeTick.text = "In: " + "%03d" % (SettingsConf.S.QuickStart.respawn_delay - current_ticks)
    if current_ticks == SettingsConf.S.QuickStart.respawn_delay:
        TickTockTimer.stop()
        self.hide()
